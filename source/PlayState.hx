...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		healthBar.createFilledBar(FlxColor.fromString('#' + dad.iconColor), FlxColor.fromString('#' + boyfriend.iconColor));
		// healthBar
		healthBar.alpha = 0.45;
		add(healthBar);
		*/

		//health bar before healthbar bg? lets see 

		//remove later if nescessary, idk how to spell that again
		//it crashes weird
		//why tf am i commenting all shit
		//smart ass

		//very happy done good shit
		...
		//ill leave it just incase ill come up with smth different

		// 27th July.... LOL I HAVE AN IDEA, BUT IM CONFUSED EHEHHEHEHHE
				...
				case "winter-horrorland":
					...
					new FlxTimer().start(0.1, function(tmr:FlxTimer)
					{
						...
						new FlxTimer().start(0.8, function(tmr:FlxTimer)
						{
							camHUD.visible = true;
							remove(blackScreen);
							FlxTween.tween(FlxG.camera, {zoom: defaultCamZoom}, 1.2, {
	...
	private function popUpScore(daNote:Note):Void
		{
			...
			if (!curStage.startsWith('school'))
			{
				rating.setGraphicSize(Std.int(rating.width * 0.9));
			...
			else
			{
				...
				comboSpr.setGraphicSize(Std.int(comboSpr.width * daPixelZoom * 1.3));
			}
			...
			{
				rating.setGraphicSize(Std.int(rating.width * daPixelZoom * 0.9));
				comboSpr.setGraphicSize(Std.int(comboSpr.width * daPixelZoom * 1.3));
			}
			...
			{
				rating.setGraphicSize(Std.int(rating.width * daPixelZoom * 0.9));
				comboSpr.setGraphicSize(Std.int(comboSpr.width * daPixelZoom * 0.8));
			}
		...
		private function keyShit():Void // I've invested in emma stocks
			{
				...
				playerStrums.forEach(function(spr:FlxSprite)
				{
					...
					/*if (spr.animation.curAnim.name == 'confirm' && !curStage.startsWith('awwman'))
					{
						spr.centerOffsets();
						spr.offset.x -= 13;
						spr.offset.y -= 13;
						...
						spr.centerOffsets();
					if (spr.animation.curAnim.name == 'confirm' && !curStage.startsWith('cave'))
					{
						spr.centerOffsets();
						spr.offset.x -= 13;
						spr.offset.y -= 13;
